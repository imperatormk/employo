...
v-container(grid-list-xl)
  v-radio-group.justify-center(v-model="fields.jobSearch.value" v-if="loaded")
    ...
    .field-label.alternate.demiBold Where are you in your job search?
    v-layout(wrap, align-center).p50-bot
      v-flex(xs12, sm6, d-block)
        ...
        .small-label.alternate.demiBold You're in the early stages of finding an internship.
      v-flex(xs12, sm6, d-block)
        v-btn.border-round(@click="setJobSearchStatus(1)")
          v-radio(label="Actively Interviewing" :value="1")
        .small-label.alternate.demiBold You're actively seeking an internship and are ready to interview.
      v-flex(xs12, sm6, d-block)
        v-btn.border-round(@click="setJobSearchStatus(2)")
          v-radio(label="Open to Offers" :value="2")
        .small-label.alternate.demiBold You're not looking but open to hear about relevant opportunities.
      v-flex(xs12, sm6, d-block)
        v-btn.border-round(@click="setJobSearchStatus(3)")
          v-radio(label="Closed to Offers" :value="3")
        .small-label.alternate.demiBold You're not looking and don't want to hear about relevant opportunities.
    PropertyItem(:data="fields.companySize")
      .field-label.alternate.demiBold(slot="title") What is your company size preference? (# of employers)
